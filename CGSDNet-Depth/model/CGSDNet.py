...
from model.module.misc import upsample_size, ConvNormAct, CAP, HBD, FPN


class CGSDNet(nn.Module):
    def __init__(self,
                 backbone_path: str,
                 skip_top: bool = True,
    ...
    def forward(self,
        ...
        if self.boundary_layer is None:
            fpn_out_features = self.fpn(fpn_in_features)
        else:
            fpn_out_features, boundary_out_features = self.fpn(fpn_in_features)
...
if __name__ == "__main__":
    ...
    net = CGSDNet(backbone_path=backbone_path, skip_top=True, context_layer=CAP, boundary_layer=HBD, act_layer=nn.ReLU,
                  device=device)
